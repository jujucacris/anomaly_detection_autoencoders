...
def executar_experimentos(tipo_experimentos):
    ...
    if (tipo_experimentos == 'experimentos_outros'):
        ...
        experimentos = [
            [
                ...
                'p2p-0.3-1-amostra0.csv',  # nome_dataset
        ...
        experimentos = pd.DataFrame(experimentos,
                                    columns=['nro_experimento', 'nome_experimento', 'funcao_f', 'funcao_g', 'nitmax',
                                             'alfa', 'no', 'nome_dataset', 'k_cv', 'tipo_experimento'])
        ...
        experimentos = pd.DataFrame({
            ...
            'funcao_f': ['sig', 'sig', 'sig', 'tan', 'tan', 'tan'],
            'funcao_g': 'sig',
            'nitmax': 500,
            'alfa': 0.8,
            'no': [10, 30, 50, 10, 30, 50],
    ...
    for index, experimento in experimentos.iterrows():
        ...
        f = open(os.path.join("resultados", "exp%s_matrizes_saida_%s.csv" % (
        experimento['nro_experimento'], experimento['nome_dataset'])),
                 "w")  # abrindo o arquivo de saídas de matrizes de confusão
